<#
    ...
    .PARAMETER ImportPath
        If specified with PmirinAction Import, we import this path, instead of the target (or target parent if target is a file)
    ...
    .EXAMPLE
        ...
        @{
            'notepad' = @{
                ...
                Source = 'C:\windows\notepad.exe'
                Target = 'C:\PmirinPesterTest'
            }
        }

        # Copy C:\Windows\Notepad.exe to C:\PmirinPesterTest\notepad.exe
...
foreach($Source in @($Sources))
{
    ...
    else
    {
        ...
        if($TargetHash -ne $SourceHash)
        {
            ...
            if($PmirinAction -like 'Test' -and $PmirinAction.count -eq 1)
            {
                $TestOutput += $false
            }
        }
...
if($PmirinAction -like 'Import')
{
    ...
    Import-PmirinModule $ImportPath
}
