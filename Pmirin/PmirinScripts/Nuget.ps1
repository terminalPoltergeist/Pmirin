<#
    ...
    .EXAMPLE
        ...
        # Installs the list of Nuget packages from Nuget.org using the Global PmirinOptions to limit repetition. Packages will be downloaded to the Staging directory in the current working directory. Since the DLL included with Portable.BouncyCastle is actually named 'BouncyCastle.Crypto', we specify that in the parameters.
...
{
    Write-Error "Nuget requires Nuget.exe.  Ensure this is in your path, or explicitly specified in $ModuleRoot\Pmirin.Config's NugetPath.  Skipping [$DependencyName]"
}
...
if(Test-Path $PackagePath)
{
    ...
    if($PmirinAction -contains 'Install')
    {
        if($Force)
        {
...
if( $PmirinAction -contains 'Test' -and $PmirinAction.count -eq 1)
{
    return $False
}
